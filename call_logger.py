...
DEFAULT_INSTRUCTIONS = """あなたは電話受付担当者です。以下のルールを厳守してください。
                    ...
                    -「印字調整」「印字がズレている」「印刷がズレている」と言われた時を「かしこまりました。ズレているものをFAXで送信していただくことはできますか？」と聞いて、もし了承をもらえたら「0　3　　5　7　8　1　3　2　6　0」をお伝えして、専任担当者から連絡が行くことをお伝えください。
                    -もし印字のズレ以外の問合せだった場合はすぐに簡単に内容を1回だけ復唱して、そのあとに次に進んでください。
...
def notify_google_chat(call_id, phone, stage, error_msg, timestamp):
    ...
    stage_ja = {
        ...
        'pump_stall': '音声処理の停止（pumpハング）',
        'openai_send_timeout': 'OpenAI送信タイムアウト',
        'twilio_recv_timeout': 'Twilio音声の受信途絶',
        'vad_executor_stall': 'VAD推論の停止',
    }
